...
function NavbarInner() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50">
      {/* ── Header principal ── */}
      <div className="bg-fondo-base/95 backdrop-blur-md border-b border-white/5">
      <div className="max-w-7xl mx-auto px-12 py-5 flex items-center justify-between border-b border-white/5">
        ...
        <div className="hidden md:flex items-center gap-3" ref={dropdownRef}>
          ...
          <a
            ...
            className="group flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold border border-purple-400/60 text-texto-secundario hover:text-texto-titulo hover:border-purple-400/80 bg-marca-principal/5 hover:bg-marca-principal/10 transition-all duration-200 no-underline"
          >
            <img src="/images/logo-murcielagos-malaga.png" alt="Murciélagos Málaga" className="h-5 w-auto invert opacity-70 group-hover:opacity-100 transition-opacity" />
            ...
            <a
              ...
              className={`px-3.5 py-1.5 rounded-lg text-sm font-medium transition-colors duration-200 no-underline ${
                pathname === link.to
                  ...
                  : 'text-white hover:text-marca-principal hover:bg-white/5'
              }`}
            >
              {link.label}
            </a>
      ...
      {menuAbierto && (
        <div className="md:hidden bg-fondo-secundario border-t border-white/5 p-6 flex flex-col gap-4">
          {navLinks.map((link) => (
            <a key={link.to} href={link.to} className="text-sm font-medium text-white hover:text-marca-principal no-underline">
              {link.label}
            </a>
          ))}
          <div className="border-t border-white/10 pt-4 flex flex-col gap-3">
            ...
            {unirseOpciones.map((op) => (
              <a key={op.to} href={op.to} className="text-sm font-medium text-white hover:text-marca-principal no-underline">{op.label}</a>
            ))}
          </div>
          <div className="border-t border-white/10 pt-4 flex flex-col gap-3">
