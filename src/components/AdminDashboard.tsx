...
function PanelDetalle({
  ...
  const revisiones = refugio.observaciones
    ? parseRevisiones(refugio.observaciones)
    : [];
  const ubicacionTexto = refugio.observaciones?.split("/")[0]?.trim() || "—";
  // Estados para manejar los modales de edición/creación (si los usas)
  // const [refugioAEditar, setRefugioAEditar] = useState(null);
    ...
    >
      ...
      >
        <div>
          ...
          <p
            style={{ color: "#6b7280", fontSize: "13px", margin: "4px 0 0 0" }}
          >
            {ubicacionTexto}
          </p>
        </div>
      ...
      >
        ...
        >
          {[
            ...
            {
              label: "Altitud",
              value: refugio.altitud_metros
                ? `${refugio.altitud_metros}m`
                : null,
            },
            {
              label: "Instalación",
                  ...
                  {esUltima && (
                    <span
                      style={{
                        ...
                        color: "#1fe1a7",
                        fontWeight: 700,
                        background: "rgba(31,225,167,0.1)",
                        padding: "2px 8px",
                        borderRadius: "999px",
                      }}
                    >
...
function MapaAdmin({
  ...
  useEffect(() => {
    ...
    import("./MapaAdmin.tsx")
      .then((m) => setMapComp(() => m.default))
      .catch((err) => console.error("Error cargando MapaAdmin:", err));
  }, []);
...
function DashboardContent() {
  ...
  const [refugioAEditar, setRefugioAEditar] = useState<Refugio | null>(null);

  // EXTRAEMOS LA FUNCIÓN DE RECARGA
  const recargarRefugios = async () => {
    setLoadingRefugios(true);
    try {
      const { data, error } = await supabase
        .from("refugios")
        .select(
          "id, codigo, nombre, municipio, provincia, latitud, longitud, tipo_refugio, orientacion, material, soporte, altitud_metros, fecha_instalacion, observaciones, activo",
        )
  ...
  useEffect(() => {
    async function fetchMe() {
      ...
      try {
        // 2. Traemos el perfil público y su rol de la base de datos
        const { data, error } = await supabase
          .from("usuarios")
          .select("*, roles(nombre)")
          .eq("id", user.id)
        ...
        if (data) {
          // 3. REGISTRO EXITOSO: Guardamos sus datos.
          // Al tener datos, ya sabemos que "puede ingresar" a las zonas privadas.
          setCurrentUserData(data);

          // 4. AUTORIZACIÓN ESTRICTA: Solo es admin si la tabla de roles lo dice explícitamente.
          // (Antonio Capel tendrá que asignarse el rol 'admin' en la base de datos, no en el código).
          setIsAdmin(data.roles?.nombre === "administrador");
        }
      } catch (error) {
        console.error("Error validando el perfil de usuario:", error);
        setIsAdmin(false); // Por seguridad, si hay error, nadie es admin.
      }
    }
    ...
    async function fetchUsuarios() {
      try {
        ...
        const { data, error } = await supabase
          .from("usuarios")
          .select(
            "id, nombre, apellidos, correo, telefono, premium, activo, fecha_creacion",
          )
  ...
  useEffect(() => {
    async function fetchStats() {
      try {
        const { count: countRefugios } = await supabase
          .from("refugios")
          .select("*", { count: "exact", head: true });
        const { count: countObs } = await supabase
          .from("vista_observaciones_completas")
          .select("*", { count: "exact", head: true });
        setStats({ refugios: countRefugios || 0, observaciones: countObs || 0 });
      } catch (error) {
        console.error("Error cargando estadísticas del dashboard:", error);
        setStats({ refugios: 0, observaciones: 0 });
      }
    }
    fetchStats();
  }, []);
  ...
  useEffect(() => {
    async function fetchLeads() {
      ...
      try {
        ...
        const [resConsultas, resVR, resRefugios, resContactos] =
          await Promise.all([
            supabase
              .from("consultas_web")
              .select("*")
        ...
        const combinedLeads: Lead[] = [
          ...(resConsultas.data || []).map((l) => ({
            ...l,
            tipo_solicitud: l.tipo_evento || "Consulta General",
          })),
          ...(resVR.data || []).map((l) => ({
            ...l,
            tipo_solicitud: "Realidad Virtual",
            origen: "Web VR",
          })),
          ...(resRefugios.data || []).map((l) => ({
            ...l,
            tipo_solicitud: "Refugios",
            origen: "Web Refugios",
          })),
          ...(resContactos.data || []).map((l) => ({
            ...l,
            email: l.correo,
            tipo_solicitud: "Contacto/Voluntariado",
            origen: "Web Contacto",
          })),
        ].sort(
          (a, b) =>
  ...
  const refugiosFiltrados = refugios.filter(
    (r) =>
      busqueda === "" ||
  ...
  if (authLoading) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#050505", color: "#a1a1aa" }}>
        Verificando acceso...
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div
        style={{
          maxWidth: refugioSeleccionado ? "calc(100% - 460px)" : "960px",
          margin: "0 auto",
        ...
        {!isAdmin ? (
          <>
            ...
            >
              ...
              >
                Mi Perfil Científico — Módulo de Gamificación
              </h2>
            </div>
            ...
            >
              <KPICard
                label="Nivel Actual"
                value={currentUserData?.nivel_actual?.toString() || "1"}
                highlight
              />
              <KPICard
                label="XP Total"
                value={currentUserData?.xp_total?.toString() || "0"}
              />
              <KPICard
                label="Observaciones"
                value={currentUserData?.total_observaciones?.toString() || "0"}
              />
            </div>
            ...
            >
              ...
              >
                ...
                >
                  Historial de tus contribuciones al proyecto.
                </p>
              </div>
                    ...
                    {misObservaciones.length === 0 ? (
                      ...
                      misObservaciones.map((o) => (
                        ...
                        >
                          ...
                          >
                            {o.nombre_cientifico || "Murciélago sp."}
                          </td>
                          <td
                            style={{
                              padding: "16px 24px",
          ...
          <>
            ...
            >
              ...
              <KPICard
                label="Observaciones"
                value={stats.observaciones.toString()}
              />
            </div>
            ...
            >
              {["usuarios", "refugios", "solicitudes", "informacion"].map(
                (tab) => (
                  <button
                    key={tab}
                    ...
                    style={{
                      ...
                      background:
                        activeTab === tab
                          ? "rgba(31, 225, 167, 0.1)"
                          : "transparent",
                      color: activeTab === tab ? "#1fe1a7" : "#a1a1aa",
            ...
            {activeTab === "usuarios" && (
              ...
              >
                ...
                >
                  ...
                  >
                    Usuarios registrados en la plataforma MUMA.
                  </p>
                </div>
                <div style={{ overflowX: "auto" }}>
                  <table
                ...
                >
                  <div>
                    ...
                    >
                      {loadingRefugios
                        ? "Cargando..."
                        : `${refugios.length} refugios en base de datos`}
                    </p>
                  </div>
                  <div
                    style={{
                      ...
                      {(["tabla", "mapa"] as const).map((v) => (
                        <button
                          ...
                          style={{
                            ...
                            background:
                              vistaRefugios === v
                                ? "rgba(31,225,167,0.15)"
                                : "transparent",
                            color: vistaRefugios === v ? "#1fe1a7" : "#6b7280",
                            textTransform: "capitalize",
                          }}
                        >
                          {v === "tabla" ? "☰ Tabla" : "Mapa"}
                          ...
                          {refugiosFiltrados.map((r) => {
                            ...
                            const manejarEliminar = async (
                              ...
                              const confirmar = window.confirm(
                                "¿Estás seguro de que deseas eliminar este refugio de la base de datos?",
                              );
                              if (!confirmar) return;
                              ...
                              try {
                                ...
                                // Aquí debes actualizar tu estado 'refugiosFiltrados' para quitar el eliminado
                                // setRefugios(refugios.filter(r => r.id !== idRefugio));
                                alert("Refugio eliminado correctamente.");
                              } catch (error) {
                                console.error("Error al eliminar:", error);
                                alert("Hubo un error al eliminar el refugio.");
                              }
                            };

                            // 2. EDITAR
                            const manejarEditar = (refugio: any) => {
                              // Aquí normalmente abrirías un Modal pasándole los datos del refugio
                              // setRefugioAEditar(refugio);
                              console.log(
                                "Abriendo editor para:",
                                refugio.nombre,
                              );
                            };
                            return (
                              ...
                              >
                                ...
                                {/* 🔥 NUEVA CELDA DE ACCIONES (Ver, Editar, Eliminar) */}
                                <td style={{ padding: "12px 16px" }}>
                                  <div
                                    style={{
                                      display: "flex",
                                  ...
                                  >
                                    {/* Botón Ver (Tu código original) */}
                                    <button
                                      onClick={() => setRefugioSeleccionado(r)}
                                      style={{
                                        background: "transparent",
                                        border:
                                    ...
                                    <button
                                      onClick={(e) => {
                                        e.preventDefault();
                                        setRefugioAEditar(r); // 1. Inyectamos los datos en el Modal
                                        setModalAbierto(true); // 2. Hacemos visible el Modal
                                      }}
                                      style={{
                                        background: "transparent",
                                        border:
                ...
                {vistaRefugios === "mapa" && (
                  <div style={{ height: "560px" }}>
                    {loadingRefugios ? (
                      ...
                      <MapaAdmin
                        refugios={refugios}
                        onSelect={setRefugioSeleccionado}
                      />
                    )}
                  </div>
                )}
            ...
            {activeTab === "solicitudes" && (
              ...
              >
                ...
                >
                  ...
                  >
                    Bandeja de entrada unificada de solicitudes técnicas y
                    comerciales.
                  </p>
                </div>
                <div style={{ overflowX: "auto" }}>
            ...
            {activeTab === "informacion" && (
              <a
                href="https://mumabatsensor.web.app"
                target="_blank" // Esto abre el enlace en una pestaña nueva
      ...
      {refugioSeleccionado && (
        <PanelDetalle
          ...
          onClose={() => setRefugioSeleccionado(null)}
        />
      )}
      <ModalFormularioRefugio
        isOpen={modalAbierto}
        onClose={() => setModalAbierto(false)}
        refugioAEditar={refugioAEditar}
        onSuccess={recargarRefugios}
      />
    </div>
  );
}
...
function InfoCard({ label, value }: { label: string; value: string }) {
  return (
    ...
    >
      ...
      <p
        style={{ color: "#fff", fontSize: "15px", fontWeight: 500, margin: 0 }}
      >
        {value}
      </p>
    </div>
...
function KPICard({
  ...
  return (
    <div
      style={{
        background: highlight ? "rgba(31,225,167,0.05)" : "#0a0a0a",
        border: highlight
          ? "1px solid rgba(31,225,167,0.3)"
      ...
      {highlight && (
        <div
          style={{
            ...
            height: "40px",
            background:
              "radial-gradient(circle, rgba(31,225,167,0.4) 0%, transparent 70%)",
